    ...
    <form class="m-form" action="{{url('/admin/'.$route)}}" method="get" id="search_form" style="width: 98%">
        <div class="form-group m-form__group row ">
            ...
            <div class="col-4" style="padding-bottom: 10px">
                <select name="agentId" class="form-control m-input" >
                    <option value="">الوكلاء</option>
                    @foreach ($agents as $item)
                    <option value="{{ $item['id'] }}" {{ isset(request()->agentId) && request()->agentId==$item['id'] ? "selected" :"" }}  >{{ $item['name'] }}</option>
                    @endforeach
        ...
        <div class="m-portlet m-portlet--last m-portlet--head-lg m-portlet--responsive-mobile" id="main_portlet">
            <div class="m-portlet__head">
                ...
                <div class="m-portlet__head-wrapper">
					...
					@if(request()->agentId)
					<div class="m-portlet__head-tools">
                        <a href="{{url('/admin/coupon/deleteAllCopouns')}}?agentId={{ request()->agentId }}" class="m-btn m-btn m-btn--square btn btn-danger">
                            <span>
                                <span>حذف الكل</span>
                            </span>
                        </a>
