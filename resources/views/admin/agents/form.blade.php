...
@endif
    ...
    <div class="form-group m-form__group row {{ $errors->has('localValue') ? 'has-danger' : ''}}">
        <label for="name" class="col-1 col-form-label"> القيمة بالعمله المحلية   </label>
        <div class="col-9">
            <input type="text" {{ isset($show)?$show:"" }} name="localValue" class="form-control m-input"
                    placeholder="القيمة بالعمله المحلية" value="{{ (isset($row) && $row['localValue'])? $row['localValue']: old('localValue') }}">
            {!! $errors->first('localValue', '<span class="form-control-feedback">:message</span>') !!}
        </div>
    </div>
    ...
    <div class="form-group m-form__group row {{ $errors->has('active') ? 'has-danger' : ''}}">
        ...
        <div class="col-9">
            <select name="active"  class="form-control m-input">
                    ...
                    <option value="0" {{ isset($row) && $row['active']==0? "selected" : "" }}>موقوف</option>
            </select>
    ...
    <div class="form-group m-form__group row {{ $errors->has('file') ? 'has-danger' : ''}}">
        ...
        @if(isset($row['nationalId']) !='')
            ...
            <a download="{{ $row['nationalId'] }}" href="{{Config::get('app.image_url')}}/{{ $row['nationalId'] }}" title="{{ $row['nationalId'] }}" target="_blank">
                تحميل الصورة
            </a>
