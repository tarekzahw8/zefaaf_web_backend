...
<div class="row">
    <form class="m-form" action="{{url('/admin/'.$route)}}" method="get" id="search_form">
        <div class="form-group m-form__group row ">
			...
			<div class="col-4">
                <select name="countryId" id="countryId" class="form-control m-input">
                    <option value="">الدولة</option>
                    @foreach($countries as $key=>$value)
                        <option value="{{ $value['id'] }}"
                        {{ isset(request()->countryId) && request()->countryId== $value['id'] ? "selected" :"" }}
                        > {{ $value['nameAr'] }} </option>
                    @endforeach
                </select>
            </div>
            <div class="col-2">
                <select name="field" class="form-control m-input">
                    ...
                    <option name="validFor">{{ __('admin.duration') }}</option>
                    <option name="usdValue">{{ __('admin.cost') }}</option>
                </select>
            </div>
                        ...
                        <table class="table table-bordered" >
                            <thead>
                                <tr>
                                    ...
                                    <th><b>{{ __('admin.duration') }} {{ __('admin.in months') }} </b></th>
                                    <th><b>{{ __('admin.cost') }}</b></th>
									 <th><b>الدولة</b></th>
                                    <th><b>سعر الباقة</b></th>
                                    <th><b>الحالة</b></th>
